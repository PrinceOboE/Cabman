...
if (isset($_POST['login'])) {
    ...
    $emailz                      = mysqli_real_escape_string($connection, $email);
    $passwordz                   = mysqli_real_escape_string($connection, $password);
    
    $query               = "SELECT * FROM admin WHERE email_address	 = '{$emailz}' ";
    ...
    if ($emailz !== $db_email_address && $passwordz !== $db_user_password) {
        header("Location: ../sign-in-error.php");
    } else if ($emailz == $db_email_address && $passwordz == $db_user_password) {       
