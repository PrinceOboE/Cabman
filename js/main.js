...
$(document).ready(function() {
    ...
    if ($('.loading-logo.driver').length > 0) {
        setTimeout(function() {
            window.location.href = "sign-up-page-1-1.php";
        }, 3000);
    }
    ...
    $('.confirm-ride-btn').on('click', function() {
        ...
        var markerList = [{ lat: 9.0827, lng: 8.67535 },
            { lat: 9.0828, lng: 8.6752 },
            { lat: 9.0844, lng: 8.6744 },
            { lat: 9.0882, lng: 8.6775 },
        ];
...
function multipleRoute(directionsService, directionsDisplay) {
    ...
    var waypts = [{
        location: "'placeId': this.originPlaceId",
        stopover: true
    }];
    var coordinates = [{ lat: 9.0820, lng: 8.6753 }];
    var icons = {
    ...
    makeMarker({ lat: 9.0820, lng: 8.6753 }, icons.end, "Route End");
...
function showMapRequestPin() {
    ...
    makeMarker({ lat: 9.0820, lng: 8.6753 }, icons.request, "Request Pin", false);
    setTimeout(function() {
        if (!$("img[src='../images/car-start-position.png']").parent().hasClass('pulse')) {
            $("img[src='../images/car-start-position.png']").parent().addClass('pulse').css('opacity', 1);
        }
    }, 600);
}
